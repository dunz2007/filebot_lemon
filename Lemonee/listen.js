...
(async function () {
  ...
  logger.loader(` NAME BOT: ${(!global.config.BOTNAME) ? "This bot was made by Lemon" : global.config.BOTNAME}`)
  logger.loader("┗━━━━━━━━━━━━━━━━━━━━━━━━━━┛")
...
if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.ADMINBOT.includes(event.senderID)) {
    ...
    if (!find_thuebot && event.body.trim() !== `${prefix}lemonee`) {
        if (event && event.threadID) {
            return api.shareContact(`[  Thông Báo Thuê Bot  ]\n─────────────────\n❎ Nhóm của bạn chưa kích hoạt sử dụng bot, vui lòng liên hệ Admin.\n─────────────────\n🌐 Facebook: https://www.facebook.com/lemontago\n🔐 Zalo: 0966.852.850`, global.config.ADMINBOT[0], event.threadID);
        } else {
            console.error("Event or event properties are undefined for sharing contact");
            return;
        }
    }
    
    if (find_thuebot && new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) {
        if (event && event.threadID) {
            return api.shareContact(`[  Thông Báo Thuê Bot  ]\n─────────────────\n❎ Nhóm của bạn đã hết hạn thời gian sử dụng bot, vui lòng liên hệ Admin để gia hạn.\n─────────────────\n🌐 Facebook: https://www.facebook.com/lemontago\n🔐 Zalo: 0966.852.850`, global.config.ADMINBOT[0], event.threadID);
        } else {
            console.error("Event or event properties are undefined for sharing contact");
            return;
        }
