...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (body.toLowerCase() === "all") {
        for (const singleIndex in handleReply.pending) {
            api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "ᴛᴀɢᴏ 🍋" : global.config.BOTNAME}`, handleReply.pending[singleIndex].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage(`❯ Bạn đã được duyệt bot bởi Admin: fb.com/lemontago`, handleReply.pending[singleIndex].threadID));
            count += 1;
        }
        ...
        for (const singleIndex of index) {
            ...
            api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "ᴛᴀɢᴏ 🍋" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage(`❯ Admin: fb.com/lemontago`, handleReply.pending[singleIndex - 1].threadID));
            count += 1;
        }
