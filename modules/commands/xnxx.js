...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    try {
        const res = (await axios.get(`https://joshweb.click/api/xsearch?q=${encodeURIComponent(search)}`)).data;
...
module.exports.handleReply = async function ({ event, api, Currencies, Users, handleReply }) {
    ...
    api.sendMessage(`Sẽ mất 1 khoảng thời gian (1-5p) để tải video, ${(await Users.getData(event.senderID)).name} vui lòng đợi`, event.threadID , (err, info)  => setTimeout ( () => { api.unsendMessage(info.messageID) } , 10000));
