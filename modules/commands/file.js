...
module.exports.handleReply = function({ handleReply, api, event }) {
    ...
    if (!handleReply.data || !Array.isArray(handleReply.data) || handleReply.data.length === 0) {
        return api.sendMessage('Thiếu dữ liệu để có thể thực thi phản hồi lại câu trả lời của bạn.', event.threadID, event.messageID);
    }

    switch (command) {
        case 'open':
            if (indices.length === 1 && indices[0] >= 0 && indices[0] < handleReply.data.length && handleReply.data[indices[0]].info.isDirectory()) {
        ...
        case 'del':
            ...
            indices.forEach(i => {
                if (i >= 0 && i < handleReply.data.length) {
        ...
        case 'view':
            if (indices.length === 1 && indices[0] >= 0 && indices[0] < handleReply.data.length && handleReply.data[indices[0]].info.isFile()) {
                api.sendMessage({
...
function openFolder(api, event, folderPath) {
    try {
        ...
        files.forEach((file, index) => {
            ...
            if (info.isDirectory() && file === 'node_modules') {
                return;
            }
            ...
            totalSize += fileSize;
            message += `${fileList.length + 1}. ${fileType} - ${file} (${convertBytes(fileSize)})\n`;
