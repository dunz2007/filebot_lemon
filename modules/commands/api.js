...
module.exports.run = async ({ api, event, args }) => {
  try {
    ...
    const srcapi = path.join(projectHome, 'Lemonee/datajson');
    ...
    switch (args[0]) {
      case 'add': {
        ...
        api.sendMessage(`✅ Upload link lên api thành công.`, event.threadID, event.messageID);
        break;
      }
      ...
      case 'check': {
        ...
        const messageToSend = `🗂️ Tổng có ${files.length} file:\n──────────────────\n${results.join('\n')}\n\n──────────────────\n|› 📝 Tổng tất cả link: ${totalLinks}\n|› 📌 Reply STT để check link status\n|› ✏️ Reply del + STT để xóa file tương ứng`;
  ...
  switch (handleReply.type) {
    case 'choosee':
      ...
      if (!isNaN(choose)) {
        ...
        api.sendMessage("❎ Bạn không phải Admin để dùng lệnh này.", tid, mid);
      }
      break;
  }
};
