...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    const LC = args[0];
    if (!LC) return api.sendMessage("⚠️ Vui lòng nhập lệnh hợp lệ", threadID);
    ...
    if (LC === "namebox") {
        ...
        if (threadEntry) {
            ...
            if (!thread) return api.sendMessage("⚠️ Không thể lấy thông tin nhóm", threadID);
        ...
        if (!info) return api.sendMessage("⚠️ Không thể lấy thông tin nhóm", threadID);
