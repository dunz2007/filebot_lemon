...
module.exports.run = async ({ api, event, args }) => {
  if (args[0] === "on") {
    ...
    api.sendMessage("Tự động gửi video sad đã được bật.", event.threadID, event.messageID);
  } else if (args[0] === "off") {
    isVideoSadEnabled = false;
    api.sendMessage("Tự động gửi video sad đã được tắt.", event.threadID, event.messageID);
  } else {
