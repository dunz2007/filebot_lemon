...
module.exports.run = async function({ api, event, Users  , Threads}) {
  ...
  ///////////////////////////////////////////////////////
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by Lemon" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
    var mlg="Kết nối thành công\nĐã load toàn bộ lệnh"
    	return api.sendMessage(threadID,async () => {
await api.shareContact(`${mlg}`, 1162050272, threadID);
});
